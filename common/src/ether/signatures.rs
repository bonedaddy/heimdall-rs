...
pub async fn resolve_function_signature(signature: &String) -> Option<Vec<ResolvedFunction>> {
    ...
    match read_cache::<Vec<ResolvedFunction>>(&format!("selector.{}", signature)) {
        Some(cached_results) => match cached_results.len() {
            0 => return None,
    ...
    let signatures = match get_json_from_url(
        format!(
            "https://api.openchain.xyz/signature-database/v1/lookup?function=0x{}",
            &signature
        ),
        3,
    )
    ...
    // cache the results
    store_cache(
        &format!("selector.{}", signature),
        signature_list.clone(),
        None,
    );
...
pub async fn resolve_event_signature(signature: &String) -> Option<Vec<ResolvedLog>> {
    ...
    let signatures = match get_json_from_url(
        format!(
            "https://api.openchain.xyz/signature-database/v1/lookup?event=0x{}",
            &signature
        ),
        3,
    )
