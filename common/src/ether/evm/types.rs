    ...
    for solidity_type in inputs {
        ...
        if solidity_type.ends_with("]") {
            ...
            if array_type.len() == 1 {
                ...
                function_inputs.push(ParamType::FixedArray(
                    Box::new(ParamType::Tuple(array_type)),
                    size,
                ));
            }
            continue;
...
pub fn display(inputs: Vec<Token>, prefix: &str) -> Vec<String> {
    ...
    for input in inputs {
        match input {
            ...
            Token::Int(val) => {
                output.push(format!("{prefix}{} {}", "int    ".blue(), val.to_string()))
            }
            Token::Uint(val) => {
                output.push(format!("{prefix}{} {}", "uint   ".blue(), val.to_string()))
            }
