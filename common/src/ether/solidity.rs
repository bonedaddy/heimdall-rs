...
impl WrappedOpcode {
    ...
    pub fn solidify(&self) -> String {
        ...
        match self.opcode.name {
            ...
            "SHA3" => {
                solidified_wrapped_opcode.push_str(&format!(
                    "keccak256(memory[{}])",
            ...
            "CALLDATALOAD" => {
                ...
                if WORD_REGEX.is_match(&solidified_slot).unwrap() {
                    // convert to usize
                    match usize::from_str_radix(&solidified_slot.replace("0x", ""), 16) {
                        Ok(slot) => {
                            solidified_wrapped_opcode
                ...
                if memloc.contains("memory") {
                    if memloc.contains("+") {
                        ...
                        match MEMLEN_REGEX.find(&format!("memory[{}]", memloc)).unwrap() {
                            Some(_) => {
                                solidified_wrapped_opcode
                                    .push_str(format!("{}.length", memloc).as_str());
                            }
                            None => {
                                solidified_wrapped_opcode
            ...
            opcode => {
                if opcode.starts_with("PUSH") {
                    ...
                    solidified_wrapped_opcode.push_str(format!("{}", opcode).as_str());
                }
            }
        }
...
mod tests {
    ...
    fn test_solidify_add_complex() {
        ...
        let complex_add_operation = WrappedOpcode::new(
            0x01,
            vec![
                WrappedInput::Opcode(add_operation_wrapped),
